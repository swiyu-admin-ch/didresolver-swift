...
public protocol DidProtocol: AnyObject {
    ...
    func resolve(didTdwLog: String) throws -> DidDoc
}
    ...
    DidProtocol
{
    ...
    open func getUrl() throws -> String {
        return try FfiConverterString.lift(rustCallWithError(FfiConverterTypeDidResolveError.lift) {
            uniffi_didresolver_fn_method_did_get_url(self.uniffiClonePointer(), $0)
        })
    }

    open func resolve(didTdwLog: String) throws -> DidDoc {
        return try FfiConverterTypeDidDoc_lift(rustCallWithError(FfiConverterTypeDidResolveError.lift) {
            uniffi_didresolver_fn_method_did_resolve(self.uniffiClonePointer(),
                                                     FfiConverterString.lower(didTdwLog), $0)
        })
    }
}
...
public struct FfiConverterTypeDidResolveError: FfiConverterRustBuffer {
    ...
    public static func read(from buf: inout (data: Data, offset: Data.Index)) throws -> DidResolveError {
        ...
        case 2: return try .MalformedDid(
                message: FfiConverterString.read(from: &buf)
            )

        case 3: return try .InvalidDidLog(
                message: FfiConverterString.read(from: &buf)
            )
